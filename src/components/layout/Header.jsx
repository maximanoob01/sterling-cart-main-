    ...
    <>
      ...
      <header className={`sticky top-0 z-50 w-full transition-all duration-500 ${
        ...
        <div className="mx-auto flex min-h-[74px] max-w-[1420px] items-center justify-between gap-5 px-4 md:px-8">
          ...
          <Link to="/" className="shrink-0 text-text-main" aria-label="Sterling Kart home">
            <LogoMark />
          </Link>

          <div className="hidden w-full max-w-[600px] lg:block">
            <ProductSearch
              query={searchQuery}
          ...
          <div className="flex items-center gap-1 sm:gap-3">
            <span className="lg:hidden">
              <HeaderIcon onClick={() => setIsSearchOpen(!isSearchOpen)} label="Search">
              <Search size={18} />
              </HeaderIcon>
            </span>
            <button
              ...
              className="hidden h-11 items-center gap-2 rounded-full border border-[#F1D8DE] bg-[#FFF6E3] px-3.5 text-left transition-colors hover:border-[#E7BCC5] hover:bg-[#FFF0D1] lg:flex"
              aria-label="View today's silver price"
            >
              <span className="hidden text-[11px] font-semibold leading-tight text-[#55483F] xl:block">
                Silver Price
                <span className="block text-[10px] font-medium text-[#9B806B]">Today</span>
              </span>
              <span className="flex h-7 w-7 items-center justify-center rounded-full bg-[#D4527A] text-white">
            ...
            <Link to="/track-order" className="hidden lg:block">
              <HeaderIcon label="Track order" text="Track order"><PackageSearch size={19} /></HeaderIcon>
            </Link>
            {isAuthenticated ? (
              <Link to="/dashboard?tab=wishlist" className="hidden sm:block">
                <HeaderIcon label="Wishlist" text="Wishlist" count={wishlistItems.length}><Heart size={18} /></HeaderIcon>
              </Link>
            ) : (
              <button type="button" onClick={useAuth().openAuthModal} className="hidden sm:block">
                <HeaderIcon label="Wishlist" text="Wishlist" count={wishlistItems.length}><Heart size={18} /></HeaderIcon>
              </button>
            )}
            ...
            {isAuthenticated ? (
              <Link to={isAdmin ? '/admin' : '/dashboard'} className="hidden sm:block">
                <HeaderIcon label="Profile" text="Profile"><User size={18} /></HeaderIcon>
              </Link>
            ) : (
              <button type="button" onClick={useAuth().openAuthModal} className="hidden sm:block">
                <HeaderIcon label="Profile" text="Profile"><User size={18} /></HeaderIcon>
          ...
          <div className="mx-auto flex min-h-[48px] max-w-[1420px] items-center justify-between gap-5 px-8">
            <div className="flex items-center gap-1 xl:gap-3">
              {navLinks.map((link) => (
                <div key={link.name} onMouseEnter={() => setActiveDropdown(link.name)}>
                  <Link to={link.path} className={`flex items-center gap-1 px-3 py-4 text-[12px] font-semibold tracking-[0.5px] text-white/80 transition-colors hover:text-white xl:text-[13px] ${activeDropdown === link.name ? 'text-white' : ''}`}>
                    {link.name}
            ...
            <div className="flex items-center gap-1">
              <div onMouseEnter={() => setActiveDropdown('Gifts')}>
                <Link to="/shop?occasion=gifting" className={`flex items-center gap-1.5 px-3 py-4 text-[12px] font-semibold tracking-[0.5px] text-white/80 transition-colors hover:text-white xl:text-[13px] ${activeDropdown === 'Gifts' ? 'text-white' : ''}`}>
                  <Gift size={15} /> Gifts <ChevronDown size={13} className="opacity-60" />
                </Link>
              </div>
              <div onMouseEnter={() => setActiveDropdown('Offers')}>
                <Link to="/shop?badge=Bestseller" className={`flex items-center gap-1 px-3 py-4 text-[12px] font-semibold tracking-[0.5px] text-white/80 transition-colors hover:text-white xl:text-[13px] ${activeDropdown === 'Offers' ? 'text-white' : ''}`}>
                  Offers <ChevronDown size={13} className="opacity-60" />
                </Link>
              </div>
              <Link to="/about" className="px-3 py-4 text-[12px] font-semibold tracking-[0.5px] text-white/80 transition-colors hover:text-white xl:text-[13px]">Our Story</Link>
            </div>
          </div>
          <AnimatePresence>
            {activeDropdown && (
          ...
          <div className="mt-3 grid grid-cols-3 gap-3">
            {trendingProducts.map((product) => (
              <Link key={product.id} to={`/product/${product.slug}`} onClick={onClose} className="group min-w-0">
                ...
                <p className="mt-2 line-clamp-2 text-[11px] font-medium leading-4 text-[#444] group-hover:text-[#B94B68] sm:text-[12px]">{product.name}</p>
                <p className="mt-1 text-[11px] font-semibold text-text-muted">{formatPrice(product.price)}</p>
              </Link>
            ))}
          </div>
...
function LogoMark({ compact = false }) {
  return (
    <span className="flex flex-col items-center leading-none">
      <span
        className={`${compact ? 'text-[19px]' : 'text-[21px] xl:text-[24px]'} whitespace-nowrap font-[var(--font-logo)] font-semibold uppercase tracking-[0.7px]`}
      >
        Sterling Kart
      </span>
      <span className={`${compact ? 'mt-1 text-[7px]' : 'mt-1.5 text-[7px] xl:text-[8px]'} whitespace-nowrap font-sans font-bold uppercase tracking-[2.2px]`}>
        925 Silver Jewels
      </span>
    </span>
  );
...
function ProductSearch({ query, setQuery, navigate, onClose, autoFocus = false }) {
  ...
  return (
    <div ref={containerRef} className="relative">
      <form onSubmit={submitSearch} className="flex h-11 items-center gap-2 rounded-full border border-[#E5DFDC] bg-[#FAF8F7] px-4 transition-all focus-within:border-[#D9909F] focus-within:bg-bg-surface focus-within:shadow-[0_0_0_3px_rgba(217,144,159,0.12)]">
        <Search size={17} className="shrink-0 text-[#A8A8A8]" />
        <input
          value={query}
          onChange={(event) => {
          ...
          className="min-w-0 flex-1 border-none bg-transparent text-[13px] text-text-main outline-none placeholder:text-text-muted"
          autoFocus={autoFocus}
          aria-label="Search products"
        />
        {query && <button type="button" onClick={() => setQuery('')} className="text-text-muted hover:text-text-main" aria-label="Clear search"><X size={15} /></button>}
      </form>
  ...
  return (
    ...
    >
      <div className="mx-auto grid max-w-[1420px] grid-cols-[1.15fr_0.9fr_0.9fr_1.45fr]">
        ...
        <MegaColumn title="Shop by occasion" className="border-r border-[#EEEAE8] bg-bg-primary">
          {[
        ...
        <MegaColumn title="Discover your style" className="border-r border-[#EEEAE8]">
          {[
          ...
          <p className="mt-5 border-t border-[#EEEAE8] pt-4 text-[11px] font-semibold uppercase tracking-[1px] text-[#B94B68]">925 sterling silver</p>
          <p className="mt-1 text-[12px] leading-5 text-[#888]">Hallmarked pieces made for everyday wear.</p>
        </MegaColumn>
          ...
          <div className="grid grid-cols-3 gap-3">
            {featuredProducts.map((product) => (
              <Link key={product.id} to={`/product/${product.slug}`} onClick={onClose} className="group">
                ...
                <p className="mt-2 line-clamp-2 text-[12px] font-medium leading-4 text-text-main group-hover:text-[#B94B68]">{product.name}</p>
                <p className="mt-1 text-[11px] font-semibold text-text-muted">{formatPrice(product.price)}</p>
              </Link>
            ))}
          </div>
    ...
    >
      <div className="mx-auto grid max-w-[1420px] grid-cols-[0.9fr_0.85fr_1fr_1.35fr]">
        ...
        <div className="min-h-[300px] p-6">
          <h3 className="mb-4 border-b border-[#EEEAE8] pb-3 font-serif text-[17px] font-semibold text-text-main">Trending gift offers</h3>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: 0.2 }} className="absolute left-0 right-0 top-full border-b border-white/60 glass-panel shadow-2xl">
      <div className="mx-auto grid max-w-[1420px] grid-cols-[0.8fr_1.6fr]">
        ...
        <div className="p-6">
          <h3 className="mb-4 border-b border-[#EEEAE8] pb-3 font-serif text-[17px] font-semibold text-text-main">Trending offers</h3>
          <div className="grid grid-cols-3 gap-3">
...
function MegaColumn({ title, children, className = '' }) {
  return (
    <div className={`min-h-[280px] p-6 ${className}`}>
      <h3 className="mb-4 border-b border-[#EEEAE8] pb-3 font-serif text-[17px] font-semibold text-text-main">{title}</h3>
      {children}
    </div>
  );
}
  ...
  const content = (
    <>
      <span className={`relative ${attention ? 'cart-attention' : ''}`}>
        {children}
        {count > 0 && <span className="absolute -right-2 -top-2 flex h-[16px] min-w-[16px] items-center justify-center rounded-full bg-[#D4527A] px-1 text-[9px] font-bold text-white">{count}</span>}
      </span>
      {text && <span className="hidden text-[10px] font-medium text-text-muted xl:block">{text}</span>}
    </>
  );
